...
**Status:** active · git · **Gene's fork of `bradautomates/claude-video`** · origin `https://github.com/FlectumBlot/claude-video.git` (push here) · upstream `https://github.com/bradautomates/claude-video.git` (pull to diff) · forked 2026-08-13 at upstream v0.2.0

**How Claude uses it:** This is Gene's **fork** — push local/tracking commits to `origin` (FlectumBlot); it is NOT the skill's source of truth. The canonical, editable copy of the skill lives at `skill-library/skills/watch/` (per the Canonical Skills Gate) — do not treat edits here as authoritative. To update `watch`: (1) `git fetch upstream && git merge upstream/main` to pull bradautomates' latest, (2) diff `claude-video/skills/watch/` against `skill-library/skills/watch/`, (3) review the changes as security-relevant code, (4) copy accepted changes into canonical `skill-library/skills/watch/` and rebuild via the skill-library workflow, (5) push the updated mirror to `origin`. When porting, re-apply the frontmatter adaptation: the standalone `SKILL.md` puts `version, argument-hint, user-invocable, homepage, repository, author` at top level, but skill-library's validator requires them nested under `metadata:` (only `name, description, allowed-tools, license, compatibility, metadata` are allowed top-level).
